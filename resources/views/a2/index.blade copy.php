    ...
    <div class="max-w-7xl mx-auto p-4">
        ...
        <form method="GET" action="{{ route('a2.index') }}"
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('a2.index') }}" class="px-4 py-2 bg-gray-300 rounded text-sm">
                    Reset
                </a>
            </div>
                            ...
                            <td class="p-2 border text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('a2.show', $r->id_reg) }}"
                                        class="px-2 py-1 bg-indigo-700 text-white rounded text-xs" title="View">
                                        <i class="bi bi-eye"></i>
                                    </a>

                                    <a href="{{ route('a2.edit', $r->id_reg) }}"
                                        class="px-2 py-1 bg-yellow-800 text-white rounded text-xs"  title="Edit">
                                        <i class="bi bi-pencil"></i>
                                    </a>

                                    <a href="{{ route('a2.print', $r->id_reg) }}" target="_blank"
                                        class="px-2 py-1 bg-green-700 text-white rounded text-xs" title="Print">
                                        <i class="bi bi-printer"></i>
                                    </a>

                                    <form action="{{ route('a2.destroy', $r->id_reg) }}" method="POST"
                                        onsubmit="return confirm('Yakin hapus data ini?')"  title="Delete">
                                        @csrf
                                        @method('DELETE')
